...
DATA.forEach((d,i)=>{
  stage.insertAdjacentHTML('beforeend',`<div class="card"><span class="stars">${STAR.repeat(5)}</span><q>${d.q}</q><div class="who"><span class="av">${d.ini}</span><div><b>${d.n}</b><span>${d.w}</span></div></div><div class="sys">${d.s}</div></div>`);
...
function lay(){
  cards.forEach((c,i)=>{
    ...
    c.style.transform=`translateX(${d*400}px) scale(${d===0?1:.86})`;
    c.style.opacity=abs>1?0:(d===0?1:.75);
    c.style.zIndex=10-abs;
    c.style.pointerEvents=abs>1?'none':'auto';
...
document.getElementById('prev').onclick=()=>{go(-1);hold()};
document.getElementById('next').onclick=()=>{go(1);hold()};
ds.forEach((el,i)=>el.onclick=()=>{a=i;lay();hold()});
cards.forEach((c,i)=>c.onclick=()=>{if(!c.classList.contains('live')){a=i;lay();hold()}});
let t=setInterval(()=>go(1),3800),held=false;
function hold(){clearInterval(t);t=setInterval(()=>go(1),3800)}
